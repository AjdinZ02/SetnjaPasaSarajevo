...
namespace SetnjaPasaSarajevo.Services
{
        ...
        where TSearch : BaseSearchObject
    {
        ...
        protected readonly SetnjaPasaSarajevoDbContext _dbContext;

        protected BaseReadService(MapsterMapper.IMapper mapper, SetnjaPasaSarajevoDbContext dbContext)
        {
            _mapper = mapper;
        ...
        public virtual async Task<PageResult<TResponse>> GetAllAsync(TSearch? search = null)
        {
            ...
            if (search?.Page.HasValue ?? false)
            {
                query = query.Skip((search.Page.Value - 1) * search.PageSize.Value);
            }

            if (search?.PageSize.HasValue ?? false)
            {
